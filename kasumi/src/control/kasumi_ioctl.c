...
static const struct file_operations kasumi_anon_fops = {
	/* obj-y port: owner = NULL — THIS_MODULE evaluates to NULL for !MODULE,
	 * and the intent (long-lived, no THIS_MODULE refcount) matches the
	 * commentary in kasumi_proc_read_hooks.c:190,441. */
	.owner          = NULL,
...
/* obj-y port: EXPORT_SYMBOL_GPL dropped — single consumer (kasumi_main.c) is
 * in the same obj-y unit, no external .ko will ever link against it. */
