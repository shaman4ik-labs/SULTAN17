...
/*
 * obj-y port: module_param_named -> core_param. core_param is the idiomatic
 * cmdline knob for builtin code (parsed pre-initcall from the kernel cmdline).
 * Naming stays 1:1 with the upstream LKM: kasumi.no_tracepoint etc. -> just
 * "kasumi_no_tracepoint" on cmdline. MODULE_PARM_DESC is no-op for !MODULE
 * (kept as inline comment via /* */).
 */
static int kasumi_no_tracepoint_param;
core_param(kasumi_no_tracepoint, kasumi_no_tracepoint_param, int, 0600);
/* Deprecated compatibility knob; syscall hooks now patch syscall table entries directly. */

static int kasumi_skip_kallsyms_param;
core_param(kasumi_skip_kallsyms, kasumi_skip_kallsyms_param, int, 0600);
/* 1=skip kallsyms resolution, use per-symbol kprobe. For GKI compatibility. */

static int kasumi_dummy_mode_param;
core_param(kasumi_dummy_mode, kasumi_dummy_mode_param, int, 0600);
/* 1=exit immediately after init starts (for testing). */
